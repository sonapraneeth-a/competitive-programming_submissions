// clang-format off
/**
 * COPYRIGHT NOTICE
 *
 * Copyright (c) 2021. Sona Praneeth Akula <sonapraneeth.akula@gmail.com>
 *
 * This file is part of binarysearch practice(submit) in competitive programming.
 *
 * This file cannot be copied and/or distributed without the express
 * permission of the copyright owner.
 *
 */

/**
 * FILE DESCRIPTION
 *
 *        Filename: 1485__k-numbers-greater-than-or-equal-to-k/solutions.cpp
 *      Created on: 05 September 2021
 *   Last modified: 05 September 2021
 *          Author: sonapraneeth_a
 *     Description: BinarySearch submission for 'K Numbers Greater Than or Equal to K' problem
 */

/**
 * CHANGELOG
 *
 * Date (DD-MM-YYYY)            Author               Update
 * 05-09-2021               sonapraneeth_a      - Creation of file
 */
// clang-format on

namespace solution_01 {
// clang-format off
/**
 * FUNCTION DESCRIPTION
 *
 * Submission link: https://binarysearch.com/problems/k-numbers-greater-than-or-equal-to-k/submissions/
 * Status:
 * Runtime:  ms, faster than % in C++
 * Time complexity:
 * Space complexity:
 * Tags:
 * Categories:
 * Additional notes
 *  -
 */
// clang-format on
int solve(vector<int>& nums) {
    
}
}  // namespace solution_01
